...
def main():
    ...
    with sm:
        smach.StateMachine.add('FOO', smach_ros.MonitorState("/sm_reset", Empty, monitor_cb), transitions={'invalid':'BAR', 'valid':'BAR2', 'preempted':'FOO'})
        smach.StateMachine.add('BAR',bar(), transitions={'bar_succeeded':'FOO'})
        smach.StateMachine.add('BAR2',bar(), transitions={'bar_succeeded':'FOO'})
